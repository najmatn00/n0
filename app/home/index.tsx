...
const Home = () => {
  ...
  return (
    <SafeAreaView className='pt-8 '>
        <Image className='w-full' source={require("../../assets/images/headHome.png")}></Image>
        <View className=' absolute top-12 right-6 w-full '>
        <View className=''>
          <Text className='text-[#F4F4F4] text-[13px]'>چه خبر سمیرا؟</Text>
          <Text className='text-[#CCCCCC] text-[13px]'>برای یک روز هیجان انگیز آماده‌ای؟</Text>
        </View>
       {/* <View style={styles.dateContainer}>
          <Icon name="calendar" size={20} color="#333" />
          <Text style={styles.date}>{getPersianDate(selectedDate)}</Text>
       </View> */}
        <Image className=' rounded-full bg-[#E88D67]  absolute -top-2 left-10' source={require("../../assets/images/profile.png")}></Image>
    ...
    <View className='px-10'>
      ...
      <Text className=' text-[#005C78] text-[22px] font-bold text-center mt-6'>استوری های پیشنهادی امروز</Text>
      {/* نمایش نمودار پیشرفت */}
        ...
        renderItem={({ item }) => (
          ...
          <TouchableOpacity onPress={() => handleItemPress(item.id)}>
            <View style={styles.storyItem}>
              ...
              {/* آیکون چک‌باکس */}
              <Icon
                name={selectedItems.includes(item.id) ? "checkbox" : "square-outline"} // انتخاب آیکون بر اساس وضعیت انتخاب‌شده
                size={20}
                color={selectedItems.includes(item.id) ? "#006989" : "#006989"} // رنگ آیکون بر اساس وضعیت انتخاب‌شده
              />
            </View>
          </TouchableOpacity>
        )}
